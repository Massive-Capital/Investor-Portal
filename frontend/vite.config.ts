...
export default defineConfig(({ mode }) => {
  ...
  const backendPort = fromProcess || fromBackendFile || "5006"
  ...
  return {
    ...
    server: {
       ...
       * proxy runs (no cross-origin, no CORS). If you set `VITE_BASE_URL=http://localhost:5006`,
       * the browser calls the API directly from `:5177` → `:5006`; when the API is down or blocked,
       ...
       *
       * `VITE_DEV_API_PROXY` (full URL, e.g. `http://127.0.0.1:5006`) overrides the proxy target
