...
import {
  OPEN_INVESTMENT_DRAFT_STAGE_INFO_MESSAGE,
  OPEN_INVESTMENT_DRAFT_STAGE_INFO_TITLE,
} from "../utils/canActivateOpenInvestment"
import "./open-investment-esign-required-modal.css"

interface OpenInvestmentDraftStageInfoModalProps {
  open: boolean
  onConfirm: () => void
  onCancel: () => void
}

export function OpenInvestmentDraftStageInfoModal({
  open,
  onConfirm,
  onCancel,
}: OpenInvestmentDraftStageInfoModalProps) {
  const titleId = useId()
    ...
    >
      ...
      >
        <header className="open_inv_esign_modal_head">
          <span
            className="open_inv_esign_modal_icon open_inv_esign_modal_icon--info"
            aria-hidden
          >
